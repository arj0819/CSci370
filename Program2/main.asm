.data
    ...
    userHeadNeighbors:     .ascii  "@@@@"  # Used to hold neighbors of User's head when helping the System make the game's second move.
    
    systemValidNeighbors:  .ascii  "@@@@"  # Used to hold the valid neighbors around the System's head location, eliminating some work when determining where to move next.
    sysFirstValidNbrBit:   .byte  0  # Used to determine validity of the first neighbor of the System's head.
    sysSecondValidNbrBit:  .byte  0  # Used to determine validity of the second neighbor of the System's head.
    sysThirdValidNbrBit:   .byte  0  # Used to determine validity of the third neighbor of the System's head.
    sysLastValidNbrBit:    .byte  0  # Used to determine validity of the last neighbor of the System's head.
    
    sysValidNbrBits:       .byte  0  # Iterable collection of validity bits.
                           .byte  0
                           .byte  0
                           .byte  0
    
    sysFirstNbrRowNum:     .word   -1  # Holds the Row position of the first neighbor of System's head.
    sysFirstNbrColNum:     .word   -1  # Holds the Column position of the first neighbor of System's head.
    sysSecondNbrRowNum:    .word   -1  # Holds the Row position of the second neighbor of System's head.
    sysSecondNbrColNum:    .word   -1  # Holds the Column position of the second neighbor of System's head.
    sysThirdNbrRowNum:     .word   -1  # Holds the Row position of the third neighbor of System's head.
    sysThirdNbrColNum:     .word   -1  # Holds the Column position of the third neighbor of System's head.
    sysLastNbrRowNum:      .word   -1  # Holds the Row position of the last neighbor of System's head.
    sysLastNbrColNum:      .word   -1  # Holds the Column position of the last neighbor of System's head.
    
    userHeadRowNum:        .word   -1  # Holds the Row position of the User's head.
    userHeadColNum:        .word   -1  # Holds the Column position of the User's head.
    
    distFromFirstSysNbr:   .word   -1  # Holds the distance from the System's first neighbor to User's head.
    distFromSecondSysNbr:  .word   -1  # Holds the distance from the System's second neighbor to User's head.
    distFromThirdSysNbr:   .word   -1  # Holds the distance from the System's third neighbor to User's head.
    distFromLastSysNbr:    .word   -1  # Holds the distance from the System's last neighbor to User's head.
    
    distFromSysNbrs:       .word   -1  # Iterable collection of distances from System neighbors to User's head.
    					   .word   -1
    					   .word   -1
    					   .word   -1
    
    sysClosestNbrDistance: .word   10           # Holds the closest distance found in distFromSysNbrs
    sysClosestNbrDistNumTimesOccurred: .word 0  # Holds the number of times sysClosestNbrDistance was found in the collection distFromSysNbrs
...
.text
	...
	determineWhoStarts:
		...
		li $s6, '@'           # Load in '@' to $s6 to indicate that to the System that the second turn hasn't yet happened.
		li $s5, '@'           # Load in '@' to $s5 to indicate that to the System that the second turn hasn't yet happened.
		xor $a0, $a0, $a0     # Seed the random number generator.
		li $a1, 2             # 2 is upper bound, we'll get either a 1 or a 0.
		li $v0, 42            # Random num generater op code.
		syscall
		beqz $a0, systemGoesFirst  # If the random number is 0, System takes the first turn, otherwise, User takes the first turn.
		j userGoesFirst
	
	systemGoesFirst:
		jal systemRandomFirstMove  # If the system goes first, it must make a random move.
		xor $a0, $a0, $a0          # Seed generator
		li $a1, 2                  # Random between 0 and 1
		li $v0, 42
		syscall
		beqz $a0, systemPicksX     # If it's 0, the System picks to play as X, otherwise, the System picks O
		j systemPicksO
		
	systemPicksX:
		li $t0, 'O'                    # Load 'O' in $t0
		sb $t0, msgUserWins + 19       # Write the 'O' to the msgUserWins
		li $t0, 'X'                    # Load 'X' in $t0
		sb $t0, msgSystemIsFirst + 35  # Write the 'X' to the msgSystemIsFirst
		sb $t0, msgSystemWins + 21     # Write the 'X' to the msgSystemWins
		la $a0, msgSystemIsFirst       # Print the msgSystemIsFirst
		li $v0, 4
		syscall
		j systemPlacesX  # System will now place it's piece (X)
		
	systemPicksO:
		li $t0, 'X'                    # Load 'X' in $t0
		sb $t0, msgUserWins + 19       # Write the 'X' to the msgUserWins
		li $t0, 'O'                    # Load 'O' in $t0
		sb $t0, msgSystemIsFirst + 35  # Write the 'O' to the msgSystemIsFirst
		sb $t0, msgSystemWins + 21     # Write the 'O' to the msgSystemWins
		la $a0, msgSystemIsFirst       # Print the msgSystemIsFirst
		li $v0, 4
		syscall
		j systemPlacesO  # System will now place it's piece (O)
	...
	userPicksX:
		li $t0, 'O'                    # Correctly arrange the messages, just like above in systemPicksO and systemPicksX
	...
	userPicksO:
		li $t0, 'X'                    # Correctly arrange the messages, just like above in systemPicksO and systemPicksX
		sb $t0, msgSystemIsFirst + 35
		sb $t0, msgSystemWins + 21
		li $t0, 'O'
		sb $t0, msgUserWins + 19
	...
	systemPlacesX:
		...
		beq $s5, '@', systemSecondTurnX # If the System has the second turn, it will place it's piece at any of the User's head's neighbors.
		
		# BEGIN SHORTEST PATH CALCULATION FOR SYSTEM PIECE PLACEMENT
		# It should place it's piece as close as possible to the User
		li $a0, '@'                    # Reset the systemHeadNeighbors values with '@'
		sb $a0, systemHeadNeighbors+0
		sb $a0, systemHeadNeighbors+1
		sb $a0, systemHeadNeighbors+2
		sb $a0, systemHeadNeighbors+3
		...
		calcColPositionForUserHeadX:  # System is X, so User must be O.
		...
		calcRowPositionForUserHeadX:  # System is X, so User must be O.
        ...
        calcDistsForUsrHeadAndSysNbrsX:  # System is X, so User must be O.
        	lw  $t0, userHeadRowNum      # Load the User's head row number to $t0
        	lw  $t1, sysFirstNbrRowNum   # Load the System head's first neighbor's row number to $t1
        	sub $t2, $t1, $t0            # Perform a subtraction to find the distance between their rows
        	abs $t2, $t2                 # Ensure that the result is always positive
        	lw  $t0, userHeadColNum      # Load the User's head col number to $t0
        	lw  $t1, sysFirstNbrColNum   # Load the System head's first neighbor's col number to $t1
        	sub $t3, $t1, $t0            # Perform a subtraction to find the distance between their columns
        	abs $t3, $t3                 # Ensure that the result is always positive
        	add $t4, $t3, $t2            # Add the row and column distances together to find total distance between User's head and Sys's first neighbor, store in $t4
			sw  $t4, distFromFirstSysNbr # Store the resulting distance into memory for later calculations.
        	lw  $t0, userHeadRowNum      # Load the User's head row number to $t0
        	lw  $t1, sysSecondNbrRowNum  # Load the System head's second neighbor's row number to $t1
        	sub $t2, $t1, $t0            # Perform a subtraction to find the distance between their rows
        	abs $t2, $t2                 # Ensure that the result is always positive
        	lw  $t0, userHeadColNum      # Load the User's head col number to $t0
        	lw  $t1, sysSecondNbrColNum  # Load the System head's second neighbor's col number to $t1
        	sub $t3, $t1, $t0            # Perform a subtraction to find the distance between their columns
        	abs $t3, $t3                 # Ensure that the result is always positive
        	add $t5, $t3, $t2            # Add the row and column distances together to find total distance between User's head and Sys's second neighbor, store in $t5
        	sw  $t5, distFromSecondSysNbr # Store the resulting distance into memory for later calculations.
        	lw  $t0, userHeadRowNum      # Load the User's head row number to $t0
        	lw  $t1, sysThirdNbrRowNum   # Load the System head's third neighbor's row number to $t1
        	beq $t1, -1, sysDetermineClosestNbrX  # If the row num is -1, branch to determine the closest neighbor
        	sub $t2, $t1, $t0            # Perform a subtraction to find the distance between their rows
        	abs $t2, $t2                 # Ensure that the result is always positive
        	lw  $t0, userHeadColNum      # Load the User's head col number to $t0
        	lw  $t1, sysThirdNbrColNum   # Load the System head's third neighbor's col number to $t1
        	sub $t3, $t1, $t0            # Perform a subtraction to find the distance between their columns
        	abs $t3, $t3                 # Ensure that the result is always positive
        	add $t6, $t3, $t2            # Add the row and column distances together to find total distance between User's head and Sys's third neighbor, store in $t6
        	sw  $t6, distFromThirdSysNbr # Store the resulting distance into memory for later calculations.
        	lw  $t0, userHeadRowNum      # Load the User's head row number to $t0
        	lw  $t1, sysLastNbrRowNum    # Load the System head's last neighbor's row number to $t1
        	beq $t1, -1, sysDetermineClosestNbrX  # If the row num is -1, branch to determine the closest neighbor
        	sub $t2, $t1, $t0            # Perform a subtraction to find the distance between their rows
        	abs $t2, $t2                 # Ensure that the result is always positive
        	lw  $t0, userHeadColNum      # Load the User's head col number to $t0
        	lw  $t1, sysLastNbrColNum    # Load the System head's third neighbor's col number to $t1
        	sub $t3, $t1, $t0            # Perform a subtraction to find the distance between their columns
        	abs $t3, $t3                 # Ensure that the result is always positive
        	add $t7, $t3, $t2            # Add the row and column distances together to find total distance between User's head and Sys's third neighbor, store in $t7
        	sw  $t7, distFromLastSysNbr  # Store the resulting distance into memory for later calculations.
        	
        	
        sysDetermineClosestNbrX:
			li $a1, 0                   # Reset validity bits.
			sb $a1, sysFirstValidNbrBit
			sb $a1, sysSecondValidNbrBit
			sb $a1, sysThirdValidNbrBit
			sb $a1, sysLastValidNbrBit
			li $s3, 'R'                 # Load an 'R' in $s3 to signify that we're going to return to check the next neighbor after validatePiecePlacement.
			nextValidSysNbrX:
				li   $s2, 'F'  # Load 'F' into $s2, to signify that validation of this neighbor has not yet succeeded
				lb   $s0, systemHeadNeighbors($a1)  # Load the next neighbor into $s0
				beq  $s0, '@', findClosestDistAndNumTimesOccurredX  # Branch to findClosestDistAndNumTimesOccurredX if we've looked at all neighbors
				move $a3, $s0  # Move the neighbor to $a3 to keep track of its value
				jal  determinePieceIndexNum  # determine the neighbor's index
				jal  validatePiecePlacement  # validate this neighbor
				beq  $s2, 'S', insertValidSysNbrX  # if the validation succeeded, insert into collection of valid neighbors
				beq  $s2, 'F', bypassInsertValidSysNbrX  # if the validation failed, bypass the insertion
				
				insertValidSysNbrX:
					sb $a3, systemValidNeighbors($a1)  # store the valid neighbor in the collection of valid neighbors
					li $a3, 1  # use $a3 to store valid bit
					jal flipValidSysNbrBits  # Flip the current validity bit
					add $a1, $a1, 1  # increment the counter
					beq $a1, 4, findClosestDistAndNumTimesOccurredX  # if we've gone through all neighbors, branch to findClosestDistAndNumTimesOccurredO
					j nextValidSysNbrX  # Otherwise, jump back to the top to validate the next neighbor
				
				bypassInsertValidSysNbrX:
					li  $a3, '@'  # Load '@' into $a3 to indicate that this neighbor was invalid
					sb  $a3, systemValidNeighbors($a1)  # Store the '@' at this neighbor's spot in the collection
					add $a1, $a1, 1  # increment the counter
					beq $a1, 4, findClosestDistAndNumTimesOccurredX  # if we've gone through all neighbors, branch to findClosestDistAndNumTimesOccurredO
					j nextValidSysNbrX  # Otherwise, jump back to the top to validate the next neighbor
				
			findClosestDistAndNumTimesOccurredX:
				lb  $a2, sysFirstValidNbrBit  # Store all validity bits and distances into their respective collections
				sb  $a2, sysValidNbrBits+0
				lb  $a2, sysSecondValidNbrBit
				sb  $a2, sysValidNbrBits+1
				lb  $a2, sysThirdValidNbrBit
				sb  $a2, sysValidNbrBits+2
				lb  $a2, sysLastValidNbrBit
				sb  $a2, sysValidNbrBits+3
				lw  $a2, distFromFirstSysNbr
				sw  $a2, distFromSysNbrs+0
				lw  $a2, distFromSecondSysNbr
				sw  $a2, distFromSysNbrs+4
				lw  $a2, distFromThirdSysNbr
				sw  $a2, distFromSysNbrs+8
				lw  $a2, distFromLastSysNbr
				sw  $a2, distFromSysNbrs+12
				li  $a1, 10                  # Reset the sysClosestNbrDistance to the maximum possible value
				sw  $a1, sysClosestNbrDistance
				li  $a1, 1                   # Reset the sysClosestNbrDistNumTimesOccurred to 1
				sw  $a1, sysClosestNbrDistNumTimesOccurred
				li  $a1, 0                   # Reset the counter to 0
				
				#lb $a0, systemValidNeighbors+0  # Load the char into $a0.
        		#li $v0, 11                     # Load the char code into $v0.
        		#syscall                        # Print first neighbor.
        		#lb $a0, systemValidNeighbors+1  # Load the char into $a0.
        		#li $v0, 11                     # Load the char code into $v0.
        		#syscall                        # Print second neighbor.
        		#lb $a0, systemValidNeighbors+2  # Load the char into $a0.
        		#li $v0, 11                     # Load the char code into $v0.
        		#syscall                        # Print third neighbor.
        		#lb $a0, systemValidNeighbors+3  # Load the char into $a0.
        		#li $v0, 11                     # Load the char code into $v0.
        		#syscall                        # Print last neighbor.
				...
				#lw $a0, distFromSysNbrs+0  # Load the char into $a0.
        		#li $v0, 1                     # Load the char code into $v0.
        		#syscall                        # Print first neighbor.
        		#lw $a0, distFromSysNbrs+4  # Load the char into $a0.
        		#li $v0, 1                     # Load the char code into $v0.
        		#syscall                        # Print second neighbor.
        		#lw $a0, distFromSysNbrs+8  # Load the char into $a0.
        		#li $v0, 1                     # Load the char code into $v0.
        		#syscall                        # Print third neighbor.
        		#lw $a0, distFromSysNbrs+12  # Load the char into $a0.
        		#li $v0, 1                     # Load the char code into $v0.
        		#syscall                        # Print last neighbor.
				...
				#lb $a0, sysValidNbrBits+0  # Load the char into $a0.
        		#li $v0, 1                     # Load the char code into $v0.
        		#syscall                        # Print first neighbor.
        		#lb $a0, sysValidNbrBits+1  # Load the char into $a0.
        		#li $v0, 1                     # Load the char code into $v0.
        		#syscall                        # Print second neighbor.
        		#lb $a0, sysValidNbrBits+2  # Load the char into $a0.
        		#li $v0, 1                     # Load the char code into $v0.
        		#syscall                        # Print third neighbor.
        		#lb $a0, sysValidNbrBits+3  # Load the char into $a0.
        		#li $v0, 1                     # Load the char code into $v0.
        		#syscall  
        		
        		# THESE STEPS ARE REPEATED for when the System is O. Therefor, the documentation for these steps will only appear once.
				nextSysNbrDistanceX:
					mul  $a3, $a1, 4  # get the correct offset for the distance collection
					lb   $a2, distFromSysNbrs($a3)  # load the next distance into $a2
					blez $a2, goToNextSysNbrDistanceX  # If the distance is <= 0, it's invalid, go to the next one
					lw   $t0, sysClosestNbrDistance  # Load the closest distance so far into $t0
					blt  $t0, $a2, goToNextSysNbrDistanceX  # If the closest distance is < the current distance value, no change required, go to next distance
					beq  $t0, $a2, incrementNumTimesOccurredX  # If the closest distance is = to current distance value, increment number of times this distance has occurred.
					bgt  $t0, $a2, attemptReplaceClosestNbrDistanceX  # If the closest distance is > current distance value, attempt to replace the closest distance with this one.

					goToNextSysNbrDistanceX:
						add $a1, $a1, 1  # increment the counter
						beq $a1, 4, placePieceSysX  # if the counter is at 4, branch to placePieceSysX
						j nextSysNbrDistanceX  # otherwise, jump back to test the next distance
						
					incrementNumTimesOccurredX:
						lw  $t1, sysClosestNbrDistNumTimesOccurred  # Load the number of times the closest distance has occurred into $t1
						add $t1, $t1, 1  # increment the number of times this distance has occurred
						sw  $t1, sysClosestNbrDistNumTimesOccurred  # Store the updated value
						add $a1, $a1, 1  # increment the counter
						beq $a1, 4, placePieceSysX  # if the counter is at 4, branch to placePieceSysX
						j nextSysNbrDistanceX  # otherwise, jump back to test the next distance
						
					attemptReplaceClosestNbrDistanceX:
						lb   $t3, sysValidNbrBits($a1)  # Load the validity bit for this distance/neighbor into $t3
						beqz $t3, goToNextSysNbrDistanceX  # If the validity bit is 0, this is an invalid move, so the distance should not be replaced. Go to next distance
						sw  $a2, sysClosestNbrDistance  # otherwise, store this new closest distance into sysClosestNbrDistance
						li  $t2, 1  # load 1 into $t2
						sw  $t2, sysClosestNbrDistNumTimesOccurred  # This new closest distance has only occurred once so far, update sysClosestNbrDistNumTimesOccurred
						add $a1, $a1, 1  # increment the counter
						beq $a1, 4, placePieceSysX  # if the counter is at 4, branch to placePieceSysX
						j nextSysNbrDistanceX  # otherwise, jump back to test the next distance
					...
					placePieceSysX:
        				...
        				#li $v0, 4                     # Load the string code into $v0.
        				#syscall  
						#lw $a0, sysClosestNbrDistance
						#li $v0, 1
						#syscall
        				#la $a0, newline               # Load the message into $a0.
        				#li $v0, 4                     # Load the string code into $v0.
        				#syscall  
						#lw $a0, sysClosestNbrDistNumTimesOccurred
						#li $v0, 1
						#syscall
						
						li  $a2, 0  # Initialize a counter
						nextPossiblePlacementSysX:
							mul  $a3, $a2, 4  # get the correct offset value for the distance collection
							lw   $t0, distFromSysNbrs($a3)  # Load the first distance into $t0
							lb   $t1, sysValidNbrBits($a2)  # Load the first validity bit into $t1
							beqz $t1, goToNextPossiblePlacementSysX  # If the validity bit is 0, this placement is invalid, goToNextPossiblePlacement
							beq  $t1, 1, attemptToPlacePieceSysX  # Otherwise, attempt to place the piece
							
							goToNextPossiblePlacementSysX:
								add $a2, $a2, 1  # increment the counter
								beq $a2, 4, resetPlacementCounterSysX  # If the counter exceeds max num of possible neighbors, we need to reset the counter and try to place again.
								j nextPossiblePlacementSysX  # otherwise, jump to nextPossiblePlacementSysX
								
							attemptToPlacePieceSysX:
								lw  $t2, sysClosestNbrDistance  # load the closest distance that we found into $t2
								beq $t0, $t2, generateRandNumForPlacementSysX  # if the current distance == closest distance, we have found which neighbor is closest. branch to generateRandNumForPlacementSysX
								add $a2, $a2, 1  # Otherwise, increment the counter
								beq $a2, 4, resetPlacementCounterSysX  # If the counter exceeds max num of possible neighbors, we need to reset the counter and try to place again.
								j nextPossiblePlacementSysX  # otherwise, jump to nextPossiblePlacementSysX
								
							# This is my favorite part of my algorithm. The number of times the closest distance was found is the upper bound of the random number generator.
							# For instance, if we found 2 neighbors with a distance of 5, then the random number generator will generate a 0 or a 1.
							# If the number is 0, then we place the piece at the current location as determined by offset counter $a2, specifically, systemValidNeighbors($a2).
							# This guarantees that the System's placement selection is truly random in a situation where multiple valid neighbors are the same distance away from User's head.
							generateRandNumForPlacementSysX:
								xor $a0, $a0, $a0  # seed the generator
								lw  $a1, sysClosestNbrDistNumTimesOccurred  # Upper bound is the number of times the closest distance occurred, making the selection of where the System places its piece truly random.
								li  $v0, 42  # random num op code
								syscall
								beqz $a0, placePieceOnBoardSysX  # If the random number generated is 0, 
								add $a2, $a2, 1  # increment the counter
								beq $a2, 4, resetPlacementCounterSysX  # If the counter exceeds max num of possible neighbors, we need to reset the counter and try to place again.
								j nextPossiblePlacementSysX  # otherwise, jump to nextPossiblePlacementSysX
							
				 			resetPlacementCounterSysX:
				 				li  $a2, 0  # reset the counter to 0
				 				j nextPossiblePlacementSysX  # jump to nextPossiblePlacementSysX
				 				
				 		placePieceOnBoardSysX:
							lb  $s0, systemValidNeighbors($a2)  # Load the chosen neighbor into $s0.
							jal determinePieceIndexNum   # Jump and link to determinePieceIndexNum so we can get the chosen neighbor's index value.
							j   systemFirstTurnX         # Just to get past systemSecondTurnX
		...
		systemSecondTurnX:
            ...
            lb   $s0, neighbors($a1)     # Load the first neighbor of Sys head into $s0 so we can determine its index number.
            add  $t1, $t1, 1
            sb   $s0, userHeadNeighbors+0  # Store the first neighbor of Sys head in memory so we can pick the correct next move later.
            jal  determinePieceIndexNum  # Get the index num of the first neighbor of Sys head, returned into $s0.
            move $t0, $s0                # Store the first neighbor's index num in $t0.
            add  $a1, $a1, 1             # Increment $a1 so we can retrieve the second neighbor of Sys head.
            lb   $s0, neighbors($a1)     # Load the second neighbor of Sys head into $s0 so we can determine its index number.
            add  $t1, $t1, 1
            sb   $s0, userHeadNeighbors+1  # Store the second neighbor of Sys head in memory so we can pick the correct next move later.
            jal  determinePieceIndexNum  # Get the index num of the second neighbor of Sys head, returned into $s0.
            move $t2, $s0                # Store the second neighbor's index num in $t2.
            add  $a1, $a1, 1             # Increment $a1 so we can retrieve the third nieghbor of Sys head.
            lb   $s0, neighbors($a1)     # Load the third neighbor of Sys head into $s0 so we can determine its index number.
            li   $t4, -1                 # Load a -1 into $t4 in case we branch away to calculations, indicating no neighbor was found.
            beq  $s0, ' ', systemPlaceSecondTurnX  # If the third neighbor of Sys head is a ' ', branch to calc column positions for Sys O neighbors.
            add  $t1, $t1, 1
            sb   $s0, userHeadNeighbors+2  # Store the third neighbor of Sys head in memory so we can pick the correct next move later.
            jal  determinePieceIndexNum  # Get the index num of the third neighbor of Sys head, returned into $s0.
            move $t4, $s0                # Store the third neighbor's index num in $t4.
            add  $a1, $a1, 1             # Increment $a1 so we can retrieve the last neighbor of Sys head.
            lb   $s0, neighbors($a1)     # Load the last neighbor of Sys head into $s0 so we can determine its index number.
            ...
            beq  $s0, ' ', systemPlaceSecondTurnX  # If the third neighbor of Sys head is a ' ', branch to calc column positions for Sys O neighbors.
            add  $t1, $t1, 1
            sb   $s0, userHeadNeighbors+3  # Store the last neighbor of Sys head in memory so we can pick the correct next move later.
            jal  determinePieceIndexNum  # Get the index num of the last neighbor of Sys head, returned into $s0.
            move $t6, $s0                # Store the last neighbor's index num in $t6.
            
            #lb   $a0, userHeadNeighbors+0
            #li   $v0, 11
            #syscall
            #lb   $a0, userHeadNeighbors+1
            #li   $v0, 11
            #syscall
            #lb   $a0, userHeadNeighbors+2
            #li   $v0, 11
            #syscall
            #lb   $a0, userHeadNeighbors+3
            #li   $v0, 11
            #syscall
            
            systemPlaceSecondTurnX:
            	xor  $a0, $a0, $a0  # seed the generator
            	move $a1, $t1       # however many valid neighbors there are is the upper bound of random num generator
            	li   $v0, 42        # random num gen op code
            	syscall
            	lb  $s0, userHeadNeighbors($a0)  # Load the neighbor of User's head that is randomly determined
            	jal determinePieceIndexNum       # get that neighbor's index value
	...
	systemPlacesO:
		...
		beq $s6, '@', systemSecondTurnO # If the System has the second turn, it will place it's piece at any of the User's head's neighbors.
		...
		calcColPositionForUserHeadO:  # System is O, so User must be X.
			div  $t6, $a3             # Divide index number of User's head by 6 to retrieve the column number from hi.
		    mfhi $t7                  # Move the modulus of previous instruction into $t7 so we can calculate the row position of the User's head later.
		    
		calcRowPositionForUserHeadO:  # System is O, so User must be X.
			sw   $t7, userHeadColNum  # Store the col number of User's head so we can make room to calculate the shortest distance
			sub  $t7, $t6, $t7        # Subtract the remainder from the index number of User's head to retrieve the row position as a multiple of 6.
			div  $t7, $a3             # Divide the row position by 6 to retrieve the row number from lo.
			mflo $t6                  # Move the division of previous instruction into $t6 so we can calculate the distance from User's head to Sys's closest neighbor
        ...
        calcDistsForUsrHeadAndSysNbrsO:  # System is O, so User must be X.
        	lw  $t0, userHeadRowNum      # Load the User's head row number to $t0
        	lw  $t1, sysFirstNbrRowNum   # Load the System head's first neighbor's row number to $t1
        	sub $t2, $t1, $t0            # Perform a subtraction to find the distance between their rows
        	abs $t2, $t2                 # Ensure that the result is always positive
        	...
        	lw  $t0, userHeadRowNum      # Load the User's head row number to $t0
        	lw  $t1, sysThirdNbrRowNum   # Load the System head's third neighbor's row number to $t1
        	beq $t1, -1, sysDetermineClosestNbrO  # If the row num is -1, branch to determine the closest neighbor
        	sub $t2, $t1, $t0            # Perform a subtraction to find the distance between their rows
        	abs $t2, $t2                 # Ensure that the result is always positive
        	lw  $t0, userHeadColNum      # Load the User's head col number to $t0
        	lw  $t1, sysThirdNbrColNum   # Load the System head's third neighbor's col number to $t1
		...
		sysDetermineClosestNbrO:
			...
			nextValidSysNbrO:
				li   $s2, 'F'  # Load 'F' into $s2, to signify that validation of this neighbor has not yet succeeded
				lb   $s0, systemHeadNeighbors($a1)  # Load the next neighbor into $s0
				beq  $s0, '@', findClosestDistAndNumTimesOccurredO  # Branch to findClosestDistAndNumTimesOccurredX if we've looked at all neighbors
				move $a3, $s0  # Move the neighbor to $a3 to keep track of its value
				jal  determinePieceIndexNum  # determine the neighbor's index
				jal  validatePiecePlacement  # validate this neighbor
				beq  $s2, 'S', insertValidSysNbrO  # if the validation succeeded, insert into collection of valid neighbors
				beq  $s2, 'F', bypassInsertValidSysNbrO  # if the validation failed, bypass the insertion
				...
				insertValidSysNbrO:
					...
					jal flipValidSysNbrBits  # Flip the current validity bit
					add $a1, $a1, 1  # increment the counter
					beq $a1, 4, findClosestDistAndNumTimesOccurredO  # if we've gone through all neighbors, branch to findClosestDistAndNumTimesOccurredO
					j nextValidSysNbrO  # Otherwise, jump back to the top to validate the next neighbor
				
				bypassInsertValidSysNbrO:
					li  $a3, '@'  # Load '@' into $a3 to indicate that this neighbor was invalid
					sb  $a3, systemValidNeighbors($a1)  # Store the '@' at this neighbor's spot in the collection
					add $a1, $a1, 1  # increment the counter
					beq $a1, 4, findClosestDistAndNumTimesOccurredO  # if we've gone through all neighbors, branch to findClosestDistAndNumTimesOccurredO
			...
			findClosestDistAndNumTimesOccurredO:
				lb  $a2, sysFirstValidNbrBit  # Store all validity bits and distances into their respective collections
				sb  $a2, sysValidNbrBits+0
				lb  $a2, sysSecondValidNbrBit
				sb  $a2, sysValidNbrBits+1
				...
				nextSysNbrDistanceO: 
					mul  $a3, $a1, 4
					lb   $a2, distFromSysNbrs($a3)
					blez $a2, goToNextSysNbrDistanceO
					lw   $t0, sysClosestNbrDistance
					blt  $t0, $a2, goToNextSysNbrDistanceO
					beq  $t0, $a2, incrementNumTimesOccurredO
					bgt  $t0, $a2, attemptReplaceClosestNbrDistanceO

					goToNextSysNbrDistanceO:
						add $a1, $a1, 1
						beq $a1, 4, placePieceSysO
						j nextSysNbrDistanceO
						
					incrementNumTimesOccurredO:
						lw  $t1, sysClosestNbrDistNumTimesOccurred
						add $t1, $t1, 1
						sw  $t1, sysClosestNbrDistNumTimesOccurred
						add $a1, $a1, 1
						beq $a1, 4, placePieceSysO
						j nextSysNbrDistanceO
						
					attemptReplaceClosestNbrDistanceO:
						lb   $t3, sysValidNbrBits($a1)
						beqz $t3, goToNextSysNbrDistanceO
						sw   $a2, sysClosestNbrDistance
						li   $t2, 1
						sw   $t2, sysClosestNbrDistNumTimesOccurred
						add  $a1, $a1, 1
						beq  $a1, 4, placePieceSysO
						j nextSysNbrDistanceO
					...
					placePieceSysO:
						...
						li  $a2, 0
						nextPossiblePlacementSysO:
							mul  $a3, $a2, 4
							lw   $t0, distFromSysNbrs($a3)
							lb   $t1, sysValidNbrBits($a2)
							beqz $t1, goToNextPossiblePlacementSysO
							beq  $t1, 1, attemptToPlacePieceSysO
							
							goToNextPossiblePlacementSysO:
								add $a2, $a2, 1
								beq $a2, 4, resetPlacementCounterSysO
								j nextPossiblePlacementSysO
								
							attemptToPlacePieceSysO:
								lw  $t2, sysClosestNbrDistance
								beq $t0, $t2, generateRandNumForPlacementSysO
								add $a2, $a2, 1
								beq $a2, 4, resetPlacementCounterSysO
								j nextPossiblePlacementSysO
								
							generateRandNumForPlacementSysO:
								xor $a0, $a0, $a0
								lw  $a1, sysClosestNbrDistNumTimesOccurred
								li  $v0, 42
								syscall
								beqz $a0, placePieceOnBoardSysO
								add $a2, $a2, 1
								beq $a2, 4, resetPlacementCounterSysO
								j nextPossiblePlacementSysO
							
				 			resetPlacementCounterSysO:
				 				li  $a2, 0
				 				j nextPossiblePlacementSysO
				 				
				 		placePieceOnBoardSysO:
							lb  $s0, systemValidNeighbors($a2)
							jal determinePieceIndexNum   # Jump and link to determinePieceIndexNum so we can return here after determinePieceIndexNum is done.
		                    j   systemFirstTurnO
		...
		systemSecondTurnO:
            ...
            lb   $s0, neighbors($a1)     # Load the third neighbor of Sys head into $s0 so we can determine its index number.
            li   $t4, -1                 # Load a -1 into $t4 in case we branch away to calculations, indicating no neighbor was found.
            beq  $s0, ' ', systemPlaceSecondTurnO  # If the third neighbor of Sys head is a ' ', branch to calc column positions for Sys X neighbors.
            add  $t1, $t1, 1
            sb   $s0, userHeadNeighbors+2  # Store the third neighbor of Sys head in memory so we can pick the correct next move later.
            jal  determinePieceIndexNum  # Get the index num of the third neighbor of Sys head, returned into $s0.
            move $t4, $s0                # Store the third neighbor's index num in $t4.
            ...
            #li   $v0, 11
            #syscall
            systemPlaceSecondTurnO:
            	xor  $a0, $a0, $a0
            	move $a1, $t1
            	li   $v0, 42
            	syscall
            	lb  $s0, userHeadNeighbors($a0)
            	jal determinePieceIndexNum   
	...
	flipValidSysNbrBits:
		beq $a1, 0, flipFrstValidSysNbrBit  # if the current value in $a1 is 0, we must flip first validity bit
		beq $a1, 1, flipScndValidSysNbrBit  # if the current value in $a1 is 1, we must flip second validity bit
		beq $a1, 2, flipThrdValidSysNbrBit  # if the current value in $a1 is 2, we must flip third validity bit
		beq $a1, 3, flipLastValidSysNbrBit  # if the current value in $a1 is 3, we must flip last validity bit
						
	flipFrstValidSysNbrBit:
		sb $a3, sysFirstValidNbrBit  # store the validity bit in $a3 to sysFirstValidNbrBit
		jr $ra  # return to insertValidSysNbrX or insertValidSysNbrO
						
	flipScndValidSysNbrBit:
		sb $a3, sysSecondValidNbrBit  # store the validity bit in $a3 to sysSecondValidNbrBit
		jr $ra  # return to insertValidSysNbrX or insertValidSysNbrO
						
	flipThrdValidSysNbrBit:
		sb $a3, sysThirdValidNbrBit  # store the validity bit in $a3 to sysThirdValidNbrBit
		jr $ra  # return to insertValidSysNbrX or insertValidSysNbrO
						
	flipLastValidSysNbrBit:
		sb $a3, sysLastValidNbrBit  # store the validity bit in $a3 to sysLastValidNbrBit
		jr $ra  # return to insertValidSysNbrX or insertValidSysNbrO
						
	systemRandomFirstMove:
		xor $a0, $a0, $a0  # Seed the generator
		li $a1, 36         # Pick any index
		li $v0 42
		syscall
		move $s0, $a0      # Move it to $s0 so it can be placed on the board
		jr $ra             # Go back to where we came (systemGoesFirst)
	...
	newGame:
		...
		li $a0, 0
		resetNeighborValues:
			sb $t0, userHeadNeighbors($a0)
			sb $t0, systemHeadNeighbors($a0)
			sb $t0, systemValidNeighbors($a0)
			add $a0, $a0, 1
			beq $a0, 4, resetCoordDistValidValues
		resetCoordDistValidValues:
	...
	validatePiecePlacement:
            ...
            beq $s3, 'R', returnToCheckNextNeighbor  # If there's a R in $s3 (as set by checkWinCondition or sysDetermineClosestNbrO), we're using validatePiecePlacement to check valid neighbors.
        ...
        validationSuccess:
            li $s2, 'S'  # Store an 'S' in $s2 for successful validation. Used for checkWinCondition (meaning the game should continue), and checking valid System neighbors.
            jr $ra       # Jump back to the caller (could be placeO, placeX, nextNeighborO, or nextNeighborX).
            
        returnToCheckNextNeighbor:
        	jr $ra  # Jump back to $ra (which will always be in either nextNeighborO, nextNeighborX, nextValidSysNbrO, or nextValidSysNbrX).

    checkWinCondition:
    	# If the current player's (X or O) new head location has no open neighbors, 
    	# then the other player has won the game.
        li $s3, 'R'          # Store an 'R' in $s3 to flag validatePiecePlacement that we're checking win condition and want to return to check more neighbors.
    ...
    checkNeighborsX:
     ...
     winnerX:  # determine whether X is the System or the User
     	lb $t0, msgUserWins+19  
     	beq $t0, 'X', printUserWinsX
     	lb $t0, msgSystemWins+21
     	beq $t0, 'X', printSystemWinsX
     	printUserWinsX:
     ...
     winnerO:  # determine whether O is the System or the User
     	lb $t0, msgUserWins+19
     	beq $t0, 'O', printUserWinsO
     	lb $t0, msgSystemWins+21
     	beq $t0, 'O', printSystemWinsO
     	printUserWinsO:
			la $a0, msgUserWins    # Load the message address into $a0.
			li $v0, 4              # Load $v0 with code to print a string.
